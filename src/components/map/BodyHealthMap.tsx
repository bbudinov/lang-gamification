...
const WORLD_ID = "body-health";
const BG_COLOR = "#1a0a0a";

const bodyWorld = WORLDS.find((w) => w.id === WORLD_ID)!;
const BODY_CITIES = CITIES.filter((c) =>
  bodyWorld.topicIds.includes(c.topicId)
);
...
function getCityPositions(): Record<string, [number, number, number]> {
  ...
  const radius = 20;
  BODY_CITIES.forEach((city, i) => {
    const angle = (i / BODY_CITIES.length) * Math.PI * 2 - Math.PI / 2;
    positions[city.id] = [
      ...
      >
        ...
        >
          ...
          <span
            ...
            style={{
              background: unlocked ? "rgba(239,68,68,0.2)" : "rgba(0,0,0,0.5)",
              color: unlocked ? "#fca5a5" : "#666",
              border: `1px solid ${unlocked ? "rgba(239,68,68,0.3)" : "rgba(100,100,100,0.3)"}`,
            }}
          >
            {city.building[lang]}
          </span>
...
function SceneContent({
  ...
  return (
    <>
      ...
      <pointLight position={[0, 10, 0]} color="#ef4444" intensity={1} distance={50} />
      <Ground />
...
export function BodyHealthMap({ onSelectCity }: BodyHealthMapProps) {
