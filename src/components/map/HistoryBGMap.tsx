...
function getCityPositions(): Record<string, [number, number, number]> {
  ...
  const radius = 20;
  HISTORY_CITIES.forEach((city, i) => {
    const angle = (i / HISTORY_CITIES.length) * Math.PI * 2 - Math.PI / 2;
    positions[city.id] = [
      ...
      >
        ...
        >
          ...
          <span
            ...
            style={{
              background: unlocked ? "rgba(180,83,9,0.2)" : "rgba(0,0,0,0.5)",
              color: unlocked ? "#fbbf24" : "#666",
              border: `1px solid ${unlocked ? "rgba(180,83,9,0.3)" : "rgba(100,100,100,0.3)"}`,
            }}
          >
            {city.building[lang]}
          </span>
...
function SceneContent({
  ...
  return (
    <>
      ...
      <pointLight position={[0, 10, 0]} color="#b45309" intensity={1} distance={50} />
      <Ground />
...
// ─── Main export ────────────────────────────────────────────────
interface HistoryBGMapProps {
  onSelectCity: (city: City) => void;
}

export function HistoryBGMap({ onSelectCity }: HistoryBGMapProps) {
