...
const WORLD_ID = "sports-world";
const BG_COLOR = "#0a1a0a";

const sportsWorld = WORLDS.find((w) => w.id === WORLD_ID)!;
const SPORTS_CITIES = CITIES.filter((c) =>
  sportsWorld.topicIds.includes(c.topicId)
);
...
function getCityPositions(): Record<string, [number, number, number]> {
  ...
  const radius = 20;
  SPORTS_CITIES.forEach((city, i) => {
    const angle = (i / SPORTS_CITIES.length) * Math.PI * 2 - Math.PI / 2;
    positions[city.id] = [
...
function Ground() {
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -2, 0]}>
      ...
      <meshStandardMaterial color="#0d200d" roughness={1} />
    </mesh>
  );
}
      ...
      >
        ...
        >
          ...
          <span
            ...
            style={{
              background: unlocked ? "rgba(34,197,94,0.2)" : "rgba(0,0,0,0.5)",
              color: unlocked ? "#86efac" : "#666",
              border: `1px solid ${unlocked ? "rgba(34,197,94,0.3)" : "rgba(100,100,100,0.3)"}`,
            }}
          >
            {city.building[lang]}
          </span>
...
function SceneContent({
  ...
  return (
    <>
      ...
      <pointLight position={[0, 10, 0]} color="#22c55e" intensity={1} distance={50} />
      <Ground />
...
export function SportsMap({ onSelectCity }: SportsMapProps) {
